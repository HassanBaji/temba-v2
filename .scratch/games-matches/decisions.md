...
## Superseded by Friendly tournaments

- **Item 13** ("Tournament Matches add anytime; sides optional") is superseded by
  ADR-0017. Friendly tournament is now a Pool round robin whose Matches are
  generated by the Pool draw, not added by hand.
- **Item 15** ("Home/pickup/Group home list Games, not Matches") is amended by
  ADR-0018. A drawn Friendly tournament expands into one row per Pool Match the
  viewer sits on, in My Games and the Home carousel only.

See `.scratch/friendly-tournaments/decisions.md`.
