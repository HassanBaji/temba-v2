...
- **Amended for hub card Join:** `.scratch/game-card-side-join/spec.md` ([TEM-192](https://linear.app/temba-app/issue/TEM-192/join-an-individual-friendly-game-from-the-vacant-side-on-the-games-hub)) amends stories **16–18** for Games hub `GameSummaryCard` only: a vacant side is a compact **Join** button that one-taps seat-register (empty side → left then right; partial → remaining Position). Footer **Join game** is omitted on those joinable roster cards. Stories **27–28** stay (non-CTA still opens Game home; Join must not navigate). Game details still uses the join sheet. Preferred Position is not auto-submitted from the card.
