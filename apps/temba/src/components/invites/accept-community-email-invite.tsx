...
export function AcceptCommunityEmailInvite({
  token,
  isSignedIn,
  returnPath,
}: {
  ...
  const preview = api.communities.previewEmailInvite.useQuery({ token });
  const accept = api.communities.acceptEmailInvite.useMutation({
    onSuccess: (result) => {
      toast.success(
  ...
  if (preview.data?.status === "unavailable") {
    return (
      <div className="space-y-3">
        <h1 className="text-xl font-semibold text-white">Invite unavailable</h1>
        <p className="text-sm text-white/70">
          This Community cannot accept Email invites right now.
        </p>
      </div>
    );
  }
