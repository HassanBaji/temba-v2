...
function request(groupId: string, search = "") {
  return new NextRequest(
    `http://localhost/api/media/group-images/${groupId}/image${search}`,
  );
}
...
describe("GET /api/media/group-images/{groupId}/image", () => {
  beforeEach(() => {
  ...
  it("returns 404 with no-store when the object is missing", async () => {
    ...
    const response = await GET(request(GROUP_ID), context(GROUP_ID));
  ...
  it("streams the object without a session and ignores v", async () => {
    ...
    const response = await GET(
      request(GROUP_ID, "?v=1700000000000"),
      context(GROUP_ID),
    );
