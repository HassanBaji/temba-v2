...
- Venue logos require App env `AWS_ENDPOINT_URL`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_S3_BUCKET_NAME`, and `AWS_DEFAULT_REGION`. Group images still use `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, and `SUPABASE_GROUP_IMAGES_BUCKET` (ADR-0015) until TEM-238. Cloud `install` writes syntactically valid placeholders when those secrets are unset so the App can boot; live Operator logo upload and media GET need real AWS keys; live Group image upload needs real Supabase Storage keys.
