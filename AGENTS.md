...
- Venue logos also require App env `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, and `SUPABASE_VENUE_LOGOS_BUCKET` (ADR-0006). Group images use the same URL and service-role key plus `SUPABASE_GROUP_IMAGES_BUCKET` (ADR-0015). Cloud `install` writes syntactically valid placeholders when those secrets are unset so the App can boot; live Operator logo and Group image upload need real Supabase Storage keys.
